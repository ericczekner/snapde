...
export async function POST(request:NextRequest){
    ...
    const authRedirectURL = `${authURI}/v2/authorize?client_id=${clientId}&response_type=code&redirect_uri=https://snapde.ngrok.app/api/callback`;
	const cookieStore = await cookies();
	cookieStore.set("authURI", authURI);
	cookieStore.set("clientId", clientId);

	console.log(authRedirectURL);
    return NextResponse.json({status: 'ok', authRedirectURL: authRedirectURL})
}